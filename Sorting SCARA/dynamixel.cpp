...
int CDynamixel::dxl_initialize( int devIndex, int baudnum )
{
	float baudrate;	
...
void CDynamixel::dxl_terminate()
{
	dxl_hal_close();
}

void CDynamixel::dxl_tx_packet()
{
	unsigned char i;
...
void CDynamixel::dxl_txrx_packet()
{
...
int CDynamixel::dxl_get_result()
{
	return gbCommStatus;
}

void CDynamixel::dxl_set_txpacket_id( int id )
{
	gbInstructionPacket[ID] = (unsigned char)id;
}

void CDynamixel::dxl_set_txpacket_instruction( int instruction )
{
	gbInstructionPacket[INSTRUCTION] = (unsigned char)instruction;
}

void CDynamixel::dxl_set_txpacket_parameter( int index, int value )
{
	gbInstructionPacket[PARAMETER+index] = (unsigned char)value;
}

void CDynamixel::dxl_set_txpacket_length( int length )
{
	gbInstructionPacket[LENGTH] = (unsigned char)length;
}

int CDynamixel::dxl_get_rxpacket_error( int errbit )
{
...
int CDynamixel::dxl_get_rxpacket_length()
{
	return (int)gbStatusPacket[LENGTH];
}

int CDynamixel::dxl_get_rxpacket_parameter( int index )
{
	return (int)gbStatusPacket[PARAMETER+index];
}

int CDynamixel::dxl_makeword( int lowbyte, int highbyte )
{
	unsigned short word;
...
int CDynamixel::dxl_get_lowbyte( int word )
{
	unsigned short temp;
...
int CDynamixel::dxl_get_highbyte( int word )
{
	unsigned short temp;
...
void CDynamixel::dxl_ping( int id )
{
	while(giBusUsing);
...
int CDynamixel::dxl_read_byte( int id, int address )
{
	while(giBusUsing);
...
void CDynamixel::dxl_write_byte( int id, int address, int value )
{
	while(giBusUsing);
...
int CDynamixel::dxl_read_word( int id, int address )
{
	while(giBusUsing);
...
void CDynamixel::dxl_write_word( int id, int address, int value )
{
	while(giBusUsing);
