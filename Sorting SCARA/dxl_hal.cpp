...
#define LATENCY_TIME		(1) //ms	(USB2Serial Latency timer)
#define IN_TRASFER_SIZE		(512) //unsigned char



int CDxl::dxl_hal_open(int devIndex, float baudrate)
{
	// Opening device
...
void CDxl::dxl_hal_clear(void)
{
	// Clear communication buffer
	PurgeComm(ghSerial_Handle, PURGE_RXABORT | PURGE_RXCLEAR);
}

int CDxl::dxl_hal_tx(unsigned char *pPacket, int numPacket)
{
	// Transmiting date
...
int CDxl::dxl_hal_rx(unsigned char *pPacket, int numPacket)
{
	// Recieving date
	// *pPacket: data array pointer
	// numPacket: number of data array
...
void CDxl::dxl_hal_set_timeout(int NumRcvByte)
{
	// Start stop watch
